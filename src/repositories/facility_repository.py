...
from src.schemas.facility import FacilityOut, FacilityIn, FacilityTypeOut, WorkshopOut
...
class FacilityRepository:
    ...
    def create(self, new_facility: FacilityIn) -> Optional[FacilityOut]:
        ...
        """,
            (
                new_facility.name,
                new_facility.type.facility_type_id,
                new_facility.workshop.workshop_id,
            ),
        )
        ...
        if fetched_row:
            return FacilityOut(
                ...
                type=FacilityTypeOut(
                    facility_type_id=fetched_row[2], name=fetched_row[3]
                ),
                workshop=WorkshopOut(workshop_id=fetched_row[4], name=fetched_row[5]),
            )
        else:
            return None
    ...
    def update(
        self, facility_id: int, new_facility: FacilityIn
        ...
        if fetched_row:
            type = new_facility.type
            workshop = new_facility.workshop
            return FacilityOut(fetched_row[0], fetched_row[1], type, workshop)
        else:
            return None
    ...
    def delete(self, facility_id: int) -> bool:
        ...
        cursor.execute(
            """DELETE FROM facility WHERE facility_id = %s""", (facility_id,)
        )
        self.__connection.commit()
