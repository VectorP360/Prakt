...
from src.schemas.scada_scheme import ScadaSchemeOut, ScadaSchemeIn, FacilityOut
...
class ScadaSchemeRepository:
            ...
            return ScadaSchemeOut(
                ...
                facility=FacilityOut(
                    ...
                    type=FacilityTypeOut(
                        facility_type_id=fetched_row[4], name=fetched_row[5]
                    ),
                    workshop=WorkshopOut(
                        workshop_id=fetched_row[6], name=fetched_row[7]
                    ),
                ),
        ...
        for record in cursor.fetchall():
            new_obj = ScadaSchemeOut(
                ...
                facility=FacilityOut(
                    ...
                    name=record[3],
                    type=FacilityTypeOut(facility_type_id=record[4], name=record[5]),
                    workshop=WorkshopOut(workshop_id=record[6], name=record[7]),
                ),
                content=record[8],
            )
    ...
    def update(
        self, scada_scheme_id: str, scada_in: ScadaSchemeIn
    ) -> Optional[ScadaSchemeOut]:
        cursor = self.__connection.cursor()
    ...
    def delete(self, scada_scheme_id: int) -> bool:
        ...
        cursor.execute(
            """DELETE FROM scada_scheme WHERE scada_scheme_id = %s""",
            (scada_scheme_id,),
        )
        self.__connection.commit()
