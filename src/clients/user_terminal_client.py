...
from src.handlers.name_handler import NameHandler
from src.handlers.surname_handler import SurnameHandler
from src.handlers.fathersname_handler import FathresnameHandler
from src.handlers.now_post_handler import NowPostHandler
from src.handlers.new_post_handler import NewPostHandler
...
class UserTerminalClient:
        ...
        try:
            ...
            if new_hire == Command.NEW_HIRE_USER:
                ...
                hire_date = datetime.strptime(
                    str(input("Дата найма сотрудника (год-месяц-число): ")), "%y-%m-%d"
                )
    ...
    def show_user(self):
        try:
            select_type = int(
                input(
                    "Вы хотите просмотреть одну запись с конкретным ID или сразу все?\n1: C конкретным ID\n2: Все записи\n"
                )
            )
    ...
    def delete_user(self):
        try:
            ...
            for iteration in self.user_repository.get_all():
                print(
                    f"""ID: {iteration.user_id}, ФИО: {iteration.surname} {iteration.name} {iteration.fathersname}, должность: {iteration.post.name}"""
                )
                user_id.append(iteration.user_id)

            deleted_user = int(
                input("Введите ID сотрудника, чью запись хотите удалить: ")
            )
    ...
    def edit_user_from_excel(self):
        ...
        name.set_next(surname).set_next(fathersname).set_next(now_post).set_next(
            new_post
        ).set_next(now_facility).set_next(new_facility)
        ...
        for row in sheet.iter_rows(min_row=2, values_only=True):
            try:
                ...
                if row[6]:
                    fetched_facility = self.facility_repository.get_by_name(
                        name=str(row[6])
                    )
                    if fetched_facility:
    ...
    def select_facility(self) -> Optional[FacilityOut]:
        ...
        facility_id = int(
            input("На какой установке (номер установки) будет работать сотрудник?: ")
        )
