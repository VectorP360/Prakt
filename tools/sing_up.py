...
class SingUp:
    ...
    def singing_up(self) -> Optional[UserOut]:
        ...
        while check == True:
            ...
            logined_user = self.repository_manager.get_user_repository().get_by_login(login, password)

            if logined_user:
                return logined_user
            
            print('Неверный логин или пароль!\nХотите повторить попытку входа?')
            if input('(y/n) если вы ничего не введёте, будет выбран вариант n: ') == Acceptance.YES:
                pass
            else:
                return None